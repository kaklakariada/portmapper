...
class BrowserLauncher {
	...
	private static Log logger = LogFactory.getLog(BrowserLauncher.class);
	...
	public static void openURL(String url) throws IOException {
		...
		case MRJ_2_0:
			...
			break;
		case MRJ_2_1:
			execute(new String[] { (String) browser, url }, false);
			break;
		case MRJ_3_0:
		...
		case OTHER:
			...
			// Netscape
			int exitCode = execute(new String[] {
					(String) browser,
					NETSCAPE_REMOTE_PARAMETER,
					NETSCAPE_OPEN_PARAMETER_START + url
							+ NETSCAPE_OPEN_PARAMETER_END }, true);
	...
	private static int execute(String[] cmdArray, boolean waitForExit)
			throws IOException {
		logger.info("Lauching browser using command "
		...
		if (waitForExit) {
			try {
				...
				logger.warn("Got exception while waiting for browser to exit",
						e);
				return -1;
			}
		} else {
	 ...
	 */
	/*
	 * private static String getEnvironmentBrowser() { String browser = null;
	 * try { String[] echoParams = { "/bin/sh", "-c", "echo $BROWSER" }; Process
	 * echoProcess = Runtime.getRuntime().exec(echoParams); InputStream
	 * echoStream = echoProcess.getInputStream(); BufferedReader echoReader =
	 * new BufferedReader(new InputStreamReader(echoStream)); browser =
	 * echoReader.readLine(); echoReader.close(); } catch (Throwable t) { // If
	 * anything goes wrong, we'll return null. } return browser; }
	 */
